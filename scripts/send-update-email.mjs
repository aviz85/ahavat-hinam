...
const { data: profs } = await sb.from("profiles").select("id,name");
const pmap = new Map(profs.map((p) => [p.id, p.name]));
const targets = users.users.filter((u) => u.email && pmap.has(u.id));

const html = (name) => `<!doctype html><html dir="rtl" lang="he"><body style="margin:0;background:#fff7f2;font-family:Arial,sans-serif;">
<div style="max-width:520px;margin:0 auto;padding:32px 24px;text-align:center;direction:rtl;">
  ...
  <p style="font-size:14px;color:#9b8189;">avizmaeir@gmail.com</p>
  ...
  <p style="font-size:13px;color:#9b8189;margin-top:24px;">אהבת חינם · גרסת בטא · תודה שאתם חלק מהניסוי הזה ❤️</p>
</div></body></html>`;

let sent = 0, failed = 0;
for (const u of targets) {
  const name = pmap.get(u.id);
  const r = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: "אהבת חינם <noreply@hugs.photos>",
      to: [u.email],
      subject: `🧪 ${name}, יש חדש באהבת חינם — ואנחנו רוצים לשמוע ממך`,
      html: html(name),
    }),
  });
  if (r.ok) sent++;
  else { failed++; console.log("FAIL", u.email, await r.text()); }
  console.log(u.email, "->", r.ok ? "sent" : "FAILED");
